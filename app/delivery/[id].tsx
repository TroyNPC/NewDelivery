// app/delivery/[id].tsx - FIXED VERSION WITH ALL FEATURES
...
type DeliveryStatus = "in_progress" | "picked_up" | "delivered";
...
export default function DeliveryTracking() {
  ...
  const [deliveryStatus, setDeliveryStatus] = useState<DeliveryStatus>("in_progress");
  ...
  // FIXED: Fetch delivery status - only query existing fields
  const fetchDeliveryStatus = useCallback(async () => {
    ...
    try {
      // ONLY query fields that exist in your database
      const { data, error } = await supabase
        .from("deliveries")
        .select("status") // Only query status field
        .eq("id", deliveryId)
        .single();

      if (error) {
        addDebugLog(`DB Error: ${error.message}`);
        // Use params as fallback - don't throw error
        return;
      }

      if (data && data.status) {
        setDeliveryStatus(data.status as DeliveryStatus);
        await cacheDeliveryData(deliveryId, { status: data.status });
        addDebugLog(`Delivery status: ${data.status}`);
      }
    } catch (error) {
  ...
  // FIXED: Update delivery status - simplified
  const updateDeliveryStatus = async (newStatus: DeliveryStatus) => {
    ...
    try {
      ...
      const statusMessages: Record<DeliveryStatus, string> = {
        in_progress: "Delivery tracking started!",
        picked_up: "Laundry picked up successfully!",
  ...
  useEffect(() => {
    return () => {
      ...
      // WebViews will auto-cleanup with the cleanup functions in HTML
    };
  }, []);
  ...
  const statusConfig = {
    in_progress: {
      ...
      nextStatus: "picked_up" as DeliveryStatus,
    ...
    picked_up: {
      ...
      description: "You have picked up the laundry - heading to customer"
    },
    delivered: {
      ...
      nextStatus: "delivered" as DeliveryStatus,
      description: "Delivery has been successfully completed"
    }
  };
  ...
  // [Keep all your existing JSX return structure exactly as it was]
  // Only changed the WebView props to include error handling
...
const styles = StyleSheet.create({
  ...
  // Add the new status indicator style
  statusIndicator: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  // Navigation Panel Styles - ONLY FOR FULL SCREEN
  navigationPanel: {
    position: 'absolute',
  ...
  // Arrival Panel Styles - ONLY FOR FULL SCREEN
  arrivalPanel: {
    position: 'absolute',
    bottom: verticalScale(20),
    left: scale(20),
